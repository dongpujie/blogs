...
# 将项目根目录(项目根目录，blogs/)添加到 Python 的模块搜索路径中
...
if __name__ == '__main__':
    ...
    from blogapps.models import Category, Post, Tag
    ...
    # 调用ORM系统API，生成用户、分类、标签、一篇 Markdown 测试文章
    print('create a blog user')
    user = User.objects.create_superuser('admin', 'admin@hellogithub.com', 'admin')

    category_list = ['Python学习笔记', '开源项目', '工具资源', '程序员生活感悟', '测试分类']
    tag_list = ['django', 'Python', 'Pipenv', 'Docker', 'Nginx', 'Elasticsearch', 'Gunicorn', 'Supervisor', '测试标签']
    ...
    # 生成100篇中文文章
    print('create some zh_CN faked posts published within the past year')
    ...
    for post in Post.objects.all()[:40]:
